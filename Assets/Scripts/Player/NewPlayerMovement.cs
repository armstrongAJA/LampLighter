...
public class NewPlayerMovement : MonoBehaviour
{
	...
	//make an enum for the animator state
    ...
    public void Recoil(int attackDirection)
    {
        ...
        {
            Debug.Log("Recoiling");
            Data.isRecoilingVert = true;
            RB.velocity = new Vector2(RB.velocity.x, 0f);
            RB.AddForce(Vector2.up * Data.downRecoilSpeed, ForceMode2D.Impulse); //adds jump force to player
            //DoubleJump = true; //allow double jump - not reimplemented yet
        ...
        if (attackDirection == 0)//implement horizontal recoil
        {
            ...
            Data.isRecoilingHor = true;
            if (IsFacingRight)
            {
                RB.AddForce(Vector2.left * Data.horRecoilSpeed,
                            ForceMode2D.Impulse); //add force in x
            }
            if (!IsFacingRight)
            {
                RB.AddForce(Vector2.right * Data.horRecoilSpeed,
                            ForceMode2D.Impulse); //add force in x
            }
        }
    ...
    void UpdateAnimationState() //function to update the animation state
    {
        ...
        if (_moveInput.x > 0f) //If running forwards do this
        {
        ...
        else if (_moveInput.x < 0f) //If running backwards do this
        {
